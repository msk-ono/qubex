...
- **`Experiment.configure()` can be previewed for QuEL-1 and QuEL-3 systems.**
  ...
  hardware settings. QuEL-3 previews compare planned instrument deployment
  requests with fetched hardware snapshots.
...
- **`configuration_mode` now resolves as a priority-ordered control layout.**
  ...
  available", `ge-ef-fh` means "assign GE, EF, and FH", and `ge-cr-cr` now
  means "assign GE, then two CR channels."
  ...
  falling back to `ge-cr`. In `ge-ef-fh`, two-channel ports place EF and FH on
  the shared second channel.
