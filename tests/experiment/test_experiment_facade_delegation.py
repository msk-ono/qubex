...
def test_sweep_parameter_delegates_use_batch_sweep_to_measurement_service() -> None:
    """Given batch opt-in, when sweep_parameter is called, then the option is delegated."""
    exp = object.__new__(Experiment)
    measurement_stub = _MeasurementServiceStub()
    exp.__dict__["_measurement_service"] = measurement_stub
    sequence = cast(Any, lambda value: {"Q00": [value + 0.0j]})
