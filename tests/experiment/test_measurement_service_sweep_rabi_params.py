...
def test_sweep_parameter_use_batch_sweep_reads_rabi_params_only_for_swept_qubits(
    ...
    def _measure(self: MeasurementService, _seq: object, **_kwargs: object) -> Any:
        _ = self
        raise AssertionError("use_batch_sweep=True must use run_sweep_measurement")
...
def test_sweep_parameter_uses_quel3_configured_batch_default(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Given quel3 sweep config, omitted use_batch_sweep uses async sweep."""
    _stub_measurement_result_converter(monkeypatch)
