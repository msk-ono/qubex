...
class _FakeRunner:
    ...
    async def _execute_request(
        ...
        return _BackendResult(
            status={},
            data={
                "Q00": [np.array([[1.1 + 0.0j], [1.1 + 0.0j]], dtype=np.complex128)],
                "Q01": [np.array([[2.2 + 0.0j], [2.2 + 0.0j]], dtype=np.complex128)],
            },
...
def test_run_sweep_measurement_with_pack_schedules_into_single_timeline_merges_and_splits(
    ...
    assert merged_captures["Q01"][0].start_time == pytest.approx(2.0 + expected_shift)

    assert runner.build_chunks == [("Q00", "Q01")]
    assert list(result.results[0].data) == ["Q00"]
    assert list(result.results[1].data) == ["Q01"]
    assert np.asarray(result.results[0].data["Q00"][0].data).tolist() == [
        [1.1 + 0.0j],
        [1.1 + 0.0j],
    ]
    assert np.asarray(result.results[1].data["Q01"][0].data).tolist() == [
        [2.2 + 0.0j],
        [2.2 + 0.0j],
    ]
...
def test_merge_measurement_schedules_rebuilds_capture_channels_after_source_cache() -> (
    ...
    merged_captures = merged_schedule.capture_schedule.channels
    assert set(merged_captures) == {"Q00", "Q01"}
    assert merged_captures["Q01"][0].start_time == pytest.approx(106.0)


def test_merge_measurement_schedules_shifts_pulses_for_new_channels() -> None:
    """Given a later new channel, merge should shift its pulses with its captures."""
    service = MeasurementExecutionService.__new__(MeasurementExecutionService)
    first_schedule = _make_schedule(
        label="Q00", capture_start=1.0, capture_target="Q00"
    )
    ...
    q01_waveforms = merged_schedule.pulse_schedule.get_sequence(
        "Q01",
        copy=False,
    ).get_flattened_waveforms(apply_frame_shifts=False)
    assert isinstance(q01_waveforms[0], Blank)
    assert q01_waveforms[0].duration == pytest.approx(104.0)
